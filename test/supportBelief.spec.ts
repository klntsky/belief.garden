...
test.describe('Support and Unsupport a Belief', () => {
  ...
  test('Support a belief, then undo and verify it is no longer present', async ({ page }) => {
...

    {
      ...
      // Click on the support button again to unsupport
      const supportButton = beliefCard.locator('.support');
      await supportButton.click();

      // Wait for the status to update
      await page.waitForTimeout(500);

      // Verify that the belief is no longer marked as supported
      await expect(supportButton).not.toHaveClass(/selected/);
    }
  });
});
